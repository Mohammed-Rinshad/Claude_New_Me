...
export default function CinematicStory() {
  ...
  const bookScale = useTransform(F, [0.5, 0.8], [0.9, 1.05])
  const bookRot = useTransform(F, [0.5, 1], [-4, 4])
  ...
  return (
    <section id="about" ref={ref} style={{ height: '1300vh' }} className="relative">
      <div className="sticky top-0 h-[100svh] w-full overflow-hidden bg-paper">
        ...
        {/* ════════ A · THE IMBALANCE ════════ */}
        <Beat p={A} at={[0, 0.06, 0.34, 0.42]} className="text-ink">
        ...
        <Beat p={A} at={[0.4, 0.5, 0.66, 0.74]} className="text-ink">
        ...
        <Beat p={A} at={[0.72, 0.82, 1, 1]} className="text-ink">
        ...
        <Beat p={F} at={[0.63, 0.73, 1, 1]} className="text-ink !flex-row gap-10">
          <motion.img
            src="/book.webp"
            alt=""
            style={{ scale: bookScale, rotate: bookRot }}
            className="hidden sm:block w-[180px] lg:w-[220px] drop-shadow-[0_30px_50px_rgba(31,92,46,0.3)]"
          />
          <figure className="max-w-xl text-left">
